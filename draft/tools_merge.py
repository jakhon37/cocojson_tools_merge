"""
-----------------------------------------------------------------------------------
modified version of code merge.py
source: (base) jakhon37@momo:~/miniconda3/envs/labelme/lib/python3.7/site-packages/cocojson/tools$
        https://github.com/levan92/cocojson/blob/main/docs/tools/merge.md
        https://github.com/levan92/cocojson/blob/main/docs/tools/merge_from_file.md
...
def merge(jsons, img_roots, output_dir, cids=None, outname="merged"):
    ...
    for i, (json_path, images_dir_path) in enumerate(zip(jsons, img_roots)):
        ...
        imgid_old2new = {}
        ff = cocodict['images']['id']
        print(cocodict['images'])
        print(f" IMAGES['ID'] --- {ff}")

        # for img in cocodict["images"]:
        for img in range(1):
            print(f'what is in img ---  {img}')
            ...
            print('image id: ' + str(cocodict["images"]["id"]))
            print('current image id: ' + str(current_image_id))
...
def find_coco_in(grandparent, get_images=True):
    ...
    print(f'grandparent1 ==== {grandparent}')
...
def merge_from_file(merge_list, output_dir, root=('./data/')):
    """
    This is a way of merging datasets (each in coco format) based on a list file.
    `merge_list` is a txt file which contains a list of coco dataset directories to merge.
    Each coco dataset directory should have an `images` folder and a `<directory name>.json` annotation file.
    In the source list file, dataset split modes are indicated by [square brackets], followed by the constituent directory names.
    An empty lines or new square bracket will indicate the end of the previous split. Directory names must be unique.
    For e.g.:
        [train]
        iphone_dataset1
        iphone_dataset2
    ...
    `root` is the parent folder containing all the coco dataset folders (can be nested, but dataset folder names have to be unique).
     If not given, will search within `merge_list`'s folder.
    This will merge all categories in all given datasets.
    """

    merge_list_path = path(merge_list)
    print(f'merge_list_path ==== {merge_list_path}')
